...
`npm test` runs all 78 tests. The 23 unit tests each check one module, and none of them touch a container. Run them on their own with `npm run test:unit`, and they finish in about a second, even with Docker stopped.

The 55 integration tests use real Postgres, Redis and Kafka. Testcontainers starts each one in Docker for the run and throws it away after. I mocked nothing. Docker must be running when you start them with `npm run test:integration`.
...
No key carries an expiry, so `TTL` answers `-1` on each of the 5. Redis holds the only copy of a win that Kafka carries and Postgres has not recorded, and a clock that deletes it deletes the win. See [No key expires, and the stock is what bounds them](docs/architecture.md#no-key-expires-and-the-stock-is-what-bounds-them).
...
| Tests | 78 over 11 files: 23 unit, 55 integration against real Postgres, Redis and Kafka | `npm test` |
...
| Unit and integration tests | 23 unit tests in `server/test/unit/` and `web/test/unit/`, run by `npm run test:unit`. 55 integration tests in `server/test/integration/` and `web/test/integration/`, run by `npm run test:integration` against real Postgres, Redis and Kafka through testcontainers |
