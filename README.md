...
No key carries an expiry, so `TTL` answers `-1` on each of the 5. Redis holds the only copy of a win that Kafka carries and Postgres has not recorded, and a clock that deletes it deletes the win.
...
I proved the drop under load. `npm run stress` drives 10,000 buyers at 1,000 units, closes the window, then waits for the sweep. Redis held 48,416 bytes over 5 keys at the peak, in each of 12 runs. It held 0 bytes 53 ms to 359 ms later, and Postgres kept all 1,000 order rows every time. The wait is the next sweep, which runs every 250 ms.
